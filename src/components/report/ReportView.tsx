      ...
      <div className="mx-auto max-w-4xl space-y-8">
        ...
        <Section index="02 / Core" title="Your Name Numbers">
          <div className="grid gap-4 sm:grid-cols-3">
            <div>
              <h4 className="text-sm uppercase tracking-widest text-gold/80">Expression {core.expression}</h4>
              <p className="mt-2 text-sm text-white/80">
                What you're built to do in the world. The full sum of every letter in your given name reveals your operating mode.
              </p>
            </div>
            <div>
              <h4 className="text-sm uppercase tracking-widest text-gold/80">Soul Urge {core.soulUrge}</h4>
              <p className="mt-2 text-sm text-white/80">
                What your heart actually wants when no one is watching. Drawn from the vowels in your name.
              </p>
            </div>
            <div>
              <h4 className="text-sm uppercase tracking-widest text-gold/80">Personality {core.personality}</h4>
              <p className="mt-2 text-sm text-white/80">
        ...
        <Section index="04 / Cycles" title={`${cycles.personalYear} — ${copy.personalYear.title}`}>
          ...
          <div className="grid gap-2 sm:grid-cols-2">
            {cycles.months.map((m) => (
              <div
                key={m.label}
                className="flex items-center justify-between rounded-xl border border-gold/15 bg-navy/30 px-4 py-2"
              >
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            {cycles.pinnacles.map((p, i) => (
              <div key={i} className="rounded-2xl border border-gold/20 bg-navy/40 p-4">
                ...
                <p className="mt-2 text-xs text-magenta/90">
              ...
              <div key={kind} className="rounded-2xl border border-gold/20 bg-navy/40 p-4">
                ...
                <div className="flex flex-wrap gap-2">
                  {list.map((w, i) => (
                    <span
                      key={i}
                      className="rounded-full border border-gold/40 bg-gold/10 px-3 py-1 text-xs text-white"
                    >
